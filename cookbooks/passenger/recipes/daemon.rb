...
bash "install passenger/nginx from rvm" do
  user "root"
  code <<-EOH
  /usr/local/rvm/bin/rvm all exec passenger-install-nginx-module --auto --auto-download --prefix="#{nginx_path}" --extra-configure-flags="#{node[:passenger][:production][:configure_flags]}"
  EOH
  not_if "test -e #{nginx_path}"
